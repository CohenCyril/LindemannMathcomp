...
From mathcomp Require Import choice fintype bigop ssralg ssrint div ssrnum.
From mathcomp Require Import rat poly closed_field polyrcf matrix mxalgebra.
From mathcomp Require Import tuple mxpoly zmodp binomial. (*realalg.*)
...
Canonical complex_Ring :=
  Eval hnf in RingType R[i] complex_comRingMixin.
Canonical complex_comRing := 
  Eval hnf in ComRingType R[i] mulcC.
...
Canonical complex_unitRing :=
  Eval hnf in UnitRingType C ComplexFieldUnitMixin.
Canonical complex_comUnitRing :=
...
Canonical complex_iDomain :=
  Eval hnf in IdomainType R[i] (FieldIdomainMixin field_axiom).
Canonical complex_fieldMixin := FieldType R[i] field_axiom.
...
Canonical complex_numDomainType :=
  Eval hnf in NumDomainType R[i] complex_POrderedMixin.
...
Definition complex_decFieldMixin (R : rcfType) := 
  closed_fields_QEMixin (@ComplexClosed.C_acf_axiom R).
Canonical complex_decField (R : rcfType) := 
  DecFieldType R[i] (complex_decFieldMixin R).
Canonical complex_closedField (R : rcfType) := 
  ClosedFieldType R[i] (@ComplexClosed.C_acf_axiom R). 

Module ComplexNumClosed.
Section ComplexNumClosed.

Variable R : rcfType.

Definition conjc (x : R[i]) : R[i] := let: a +i* b := x in a -i* b.

Lemma conjcE a b : conjc (a +i* b) = a -i* b.
Proof. by []. Qed.
...
Fact normcCK x : x * conjc x = `|x| ^+ 2.
Proof.
move: x => [a b] /=. simpc. rewrite expr2. simpc.
rewrite -![in LHS]expr2 mulrC addNr -expr2 sqr_sqrtr //.
by apply: (addr_ge0 (sqr_ge0 _) (sqr_ge0 _)).
Qed.

Definition complex_imaginaryMixin :=
  ImaginaryMixin (@sqr_i R) normcCK.

End ComplexNumClosed.
End ComplexNumClosed.

Canonical complex_numClosedFieldType (R : rcfType) :=
  NumClosedFieldType R[i] (@ComplexNumClosed.complex_imaginaryMixin R).

Section ComplexTheories.
...
Axiom R_archimedean : Num.archimedean_axiom R.
...
have [n le_y_n] := R_archimedean (Re x).
exists n; apply (@ltr_trans _ n%:R);
  last by rewrite ltr_nat ltnSn.
...
have ltc_lt z y : ComplexField.ltc z y = (z < y); first by [].
have normc_norm z : (ComplexField.normc z)%:C = `| z |; first by [].
have lec_le z y : ComplexField.lec z y = (z <= y); first by [].
have ltrRn_ltrCn (z : R) (p : nat) : 
  ComplexField.ltc ((ComplexField.normc (z +i* 0))%:C : R[i]) p%:R = (`|z| < p%:R).
  by rewrite -natC /= eqxx [0 ^+ _]expr2 mul0r addr0 /= sqrtr_sqr.
move: le_y_n; rewrite -ltrRn_ltrCn ltc_lt normc_norm.
case: x Rx x_ge0 => a b x_real.
rewrite -lec_le /ComplexField.lec /= => /andP [/eqP -> Ha].
apply ler_lt_trans.
rewrite -lec_le /ComplexField.lec /= eqxx /=.
by rewrite [0 ^+ _]expr2 mulr0 addr0 sqrtr_sqr ler_norm.
Qed.
